...
2. **InputItem.cs**: Ctrl+SpaceキーでAutoCompleteRequestedイベントを発生させるkey handlerを追加
...
## 修正履歴: @scopeコメントannotation (2025-02-14)

**問題**: module instance階層内では上位階層のinstanceを宣言なしに参照できるが、パースや自動補完が動作しない

**原因**: コード解析时点で他のbuilding block (module, interfaceなど) への参照が解决できない

**修正内容**:
1. `CommentScopeReference`クラスを新規作成 - scope参照情报を保持
2. `NameSpace`クラスに`CommentScopeReferences`プロパティを追加
3. `CommentAnnotationItem.Parse`に`@scope`の解析を追加
   - 書式: `@scope buildingBlockName [#(.paramName(paramValue),...)] [instanceName]`
4. `GetNamedElementUpward`を扩展してcomment scopesを検索
5. `AppendAutoCompleteItem`を扩展してcomment scopesからのアイテムを追加

**对应する構文**:
```verilog
// @scope my_module                         - 直接アクセス
// @scope my_module u_inst                  - instance経由でアクセス (u_inst.signal)
// @scope my_module #(.WIDTH(8)) u_parent   - パラメータ付き

module child();
    wire [7:0] data = u_parent.data_out;  // autocompleteが動作
endmodule
```

**修正ファイル**:
- `CodeEditor2VerilogPlugin/CodeEditor2VerilogPlugin/Verilog/CommentScopeReference.cs` (新規)
- `CodeEditor2VerilogPlugin/CodeEditor2VerilogPlugin/Verilog/NameSpace.cs`
- `CodeEditor2VerilogPlugin/CodeEditor2VerilogPlugin/Verilog/Items/CommentAnnotationItem.cs`

**備考**:
- building block内ならどこからでも記述可能（先頭でなくてもよい）
- building blockの最後まで効果が続く
- パラメータ override対応
- instance名あり/なしの両方に対応
...
## 調査記録: @scopeコメントannotation - 暗黙scope指定の代替実装方針検討 (2025-02-14)

### 背景・要件
verilogの`@scope`での暗黙のscope指定について、通常の`nameSpace`に追加する形で実装し、代わりに`nameSpace`実態がないことを指定するフラグを持たせられるかを検討。
要件:
- parse/入力補完は普通に効く
- SimSetupでのファイルデータ収集時には実態がないものは無視したい

### 現状実装の整理

#### 既存の`@scope`実装
- `Verilog/CommentScopeReference.cs` - scope参照情報を保持 (BuildingBlockName, InstanceName, ParameterOverrides, ResolvedBuildingBlock)
- `Verilog/NameSpace.cs` - `CommentScopeReferences: List<>`プロパティ、`GetNamedElementFromCommentScopes`/`AppendAutoCompleteItemFromCommentScopes`を追加
- `Verilog/Items/CommentAnnotationItem.cs` - `parseScopeAnnotation()`で`@scope`を解析

#### 現在のSimSetupでのファイル収集処理
- `Data/SimulationSetup.cs` の `searchHier()` / `searchNameSpace()` が`NamedElements`を辿る
- `ModuleInstantiation`だけ再帰的に辿る
- `CommentScopeReference`はSimSetup収集対象外(`NameSpace`/`ModuleInstantiation`/`DataObject`/`InterfaceInstance`のみ列挙)
- 現状の`CommentScopeReference`設計でも、SimSetupは`@scope`対象ファイルを収集しない動作になっている

### 重要な問題点: 選択肢Bでも補完透過性が不十分

ユーザフィードバック: **選択肢Bの場合、expression parseや階層parse等、autocompleteなどで新規の階層を参照できるようにしないといけないため、全面的に修正が必要になる**

#### 課題の詳細

`@scope`で参照される他ファイルのbuilding blockに対して、以下のような子要素アクセスが補完やparseで動作する必要がある:

```verilog
// @scope my_pkg  (package参照)
// @scope my_module u_inst  (module instance参照)

module child();
    wire [7:0] data = u_inst.data_out;  // ★ u_inst.data_out のようなドット記法が補完候補に出るべき
endmodule
```

#### 影響を受けるコード箇所

`Primary.cs:391-403` - ドット記法の起点検索:
```csharp
...
`Primary.cs:685-742` - instance / namespace探索:
```csharp
...
`Primary.cs:searchNameSpace()` - 再帰的な階層探索:
- `nameSpace.NamedElements.ContainsKey()`と`nameSpace.NamedElements[word.Text]`を直接参照
- `BuildingBlock.NamedElements.ContainsIBuldingBlockInstantiation()`も直接参照

`Module.cs:AppendAutoCompleteItem()` - autocomplete出力:
```csharp
...
`NameSpace.GetNamedElementUpward()` - 上方検索
- 現状は`GetNamedElementFromCommentScopes`で検索するが、**返り値は要素であり、NameSpaceを返さない**
- つまり「`u_inst`」という識別子は解決できるが、「`u_inst.data`」の`u_inst`部分で`NameSpace`として返らない

`DataObjectReference.cs:ParseCreate` - hierarchical_identifierのパース
- `nameSpaceText`はドット記法のパス情報を保持
- `@scope`先のbuilding blockに対してドット記法でアクセスするには、`nameSpaceText`と`owner`の連携が必要

### 検討した3つの選択肢

#### 選択肢A: `NameSpace`自体にフラグを持たせる(ご提示案)

```csharp
public class NameSpace : Item, INamedElement
{
    public bool IsVirtualScope { get; init; } = false;
    public BuildingBlock? VirtualScopeTarget { get; init; } = null;
}
```

`@scope`で参照された`BuildingBlock`に対し、**ラッパー`NameSpace`インスタンス**を作成して`NamedElements`に追加:
```csharp
foreach (var scopeRef in nameSpace.CommentScopeReferences)
{
    if (scopeRef.ResolvedBuildingBlock != null)
    {
        var virtualNs = new VirtualNameSpaceWrapper(
            scopeRef.ResolvedBuildingBlock,
            scopeRef.InstanceName ?? scopeRef.BuildingBlockName);
        nameSpace.NamedElements.AddOrUpdate(virtualNs.Name, virtualNs);
    }
}
```

`VirtualNameSpaceWrapper`は:
- `NamedElements`を`targetBuildingBlock.NamedElements`に透過させる
- `GetNamedElementUpward`を委譲
- `IsVirtualScope = true`のフラグで識別
- `GetHierarchyNameSpaceDownward`で仮想的なIndex範囲を返す

**メリット**:
- parse/補完が既存ロジックを最大限再利用
- `nameSpace.NamedElements`の列挙に自動対応
- `Primary.searchNameSpace()`がそのまま動作
- `u_inst.data`のようなドット記法が透過

**デメリット**:
- `NameSpace`構築タイミングと`@scope` resolveタイミングの依存関係(BuildingBlockは別ファイルに存在)
- 解決を全パース完了後まで遅延する必要がある
- SimSetupで`IsVirtualScope`チェックが必要
- `HierarchyConnection`など他のファイル収集系も`IsVirtualScope`確認

#### 選択肢B: 全面修正 (選択肢Aの変形)

選択肢Aのラッパー`NameSpace`を作るアプローチに加えて、`NameSpace`の探索ロジックを根本的に変更:

- `NameSpace.GetNamedElementUpward()`を`@scope`先も検索対象に
- `Primary.searchNameSpace()`の`NamedElements`直接参照を、`@scope`先も含めた`GetChildElements()`的なメソッド経由に
- `Module.AppendAutoCompleteItem()`をoverrideして`@scope`先のinstance名も追加
- `HierarchyConnection`でも`@scope`参照を追跡

**メリット**:
- `@scope`参照が他ファイル探索系(`HierarchyConnection`、`CodeDocument`のcross-reference等)でも自然に動作
- 将来的な拡張性が高い

**デメリット**:
- 影響範囲が最大(`NameSpace`、`Primary`、`Module`、`BuildingBlock`、`HierarchyConnection`、`DataObjectReference`等)
- 修正量が大きい(全`NamedElements`直接参照を`GetChildElements()`に置き換える)
- テストケース追加が大量

#### 選択肢C: ハイブリッド(限定的な`NameSpace`注入)

`@scope`で参照された要素のうち**補完で展開されるべき要素のみ**を`NamedElements`に注入:
- `@scope my_module` → `my_module`の`NamedElements`の主要要素(net, variable等)のみを`NamedElements`にコピー
- `@scope my_module u_inst` → `u_inst`名で`NameSpace`ラッパー登録
- 元`BuildingBlock`は参照のみ
...
**デメリット**:
- `@scope`先の変更が反映されない(コピー時に固定)
- parameter overrideが使えない
- 設計が複雑

### 比較表 (修正版)

| 観点 | 選択肢A (ラッパーNameSpace) | 選択肢B (全面修正) | 選択肢C (要素コピー) |
|------|--------------------------|-------------------|---------------------|
| 補完透過性 | ◎ | ◎ | △ (コピー時固定) |
| parameter override | ○ (resolve時に渡す) | ○ | × |
| 影響範囲 | 中 (NameSpace中心) | 大 (全探索系) | 中 |
| 既存ロジック再利用 | ◎ | × | ○ |
| コード量 | 中 | 大 | 中 |
| 将来拡張性 | ○ | ◎ | △ |

### 結論・推奨
**選択肢A(ラッパー`NameSpace`注入)を推奨**。

理由:
1. `@scope`で参照される要素は**その参照時の状態**で固定されるのが自然(parameter overrideも参照時のスナップショット)
2. 既存の`NameSpace.NamedElements`列挙パターンに完全対応
3. SimSetupでの`IsVirtualScope`チェックは1ヶ所で済む
4. `Primary.searchNameSpace()`を含むドット記法処理がそのまま動作
5. parameter overrideも`@scope`解決時に`CommentScopeReference`の`ParameterOverrides`を渡すことで対応可能

#### 実装ステップ(選択肢Aの場合)

1. `NameSpace`に`IsVirtualScope`、`VirtualScopeTarget`、`VirtualInstanceName`プロパティ追加
2. `VirtualNameSpaceWrapper`クラス(または`NameSpace`の特殊初期化)を作成
3. `CommentAnnotationItem.parseScopeAnnotation`完了時に`@scope`参照解決をスケジュール
4. 解決タイミング: 親ファイルパース完了後、または`UpdateAsync`時に`@scope`先の`BuildingBlock`を`ProjectProperty.GetBuildingBlock()`で取得
5. 取得した`BuildingBlock`のラッパー`NameSpace`を生成し、`nameSpace.NamedElements`に追加
6. `SimulationSetup.searchNameSpace`に`IsVirtualScope`チェック追加
7. `HierarchyConnection`など他のファイル収集系でも必要に応じて`IsVirtualScope`スキップ

#### 残課題
- 解決タイミング(全ファイルパース完了前だと`@scope`先`BuildingBlock`が未登録)
- parameter overrideの反映方法
- 循環参照の防止(`@scope A`のAがさらに`@scope B`を参照)
- 削除時の伝搬(`@scope`先のファイルが削除された場合のクリーンアップ)

### 関連ファイル

| ファイル | 役割 |
|----------|------|
| `Verilog/CommentScopeReference.cs` | scope参照情報の保持 |
| `Verilog/NameSpace.cs` | `CommentScopeReferences`プロパティ、参照解決、`AppendAutoCompleteItem` |
| `Verilog/Items/CommentAnnotationItem.cs` | `@scope`パース処理 |
| `Verilog/Expressions/Primary.cs` | `searchNameSpace`、`GetNamedElementUpward`使用 |
| `Verilog/Expressions/DataObjectReference.cs` | hierarchical_identifier、TargetDataObject |
| `Verilog/BuildingBlocks/Module.cs` | `AppendAutoCompleteItem` (instance名追加) |
| `Verilog/BuildingBlocks/BuildingBlock.cs` | `BuildingBlock : NameSpace` |
| `Verilog/HierarchyConnection.cs` | 階層間接続追跡 |
| `Data/SimulationSetup.cs` | ファイルデータ収集 (searchHier/searchNameSpace) |
| `Simulation/IcarusVerilogSimulation.cs` | SimSetupの呼び出し元 |

### ステータス
- ✅ 選択肢Aで実装決定 (2025-02-14)
- 実装中

### Next Steps
- `NameSpace`に`IsVirtualScope`フラグ追加
- `CommentScopeReference`の`Resolve`をpublic化、`BuildingBlock`を返す
- 仮想NameSpace登録処理`ApplyCommentScopeReferences()`追加
- 解決タイミングの検討(parse時 vs accept時)
- `SimulationSetup.searchNameSpace`に`IsVirtualScope`チェック
- ビルド → コミット

---

## 修正履歴: @scopeアノテーションのパース時VirtualScopeNameSpace登録対応 (2025-02-14)

**問題**:
```verilog
// @scope TEST_RTL_MODULE TEST_RTL_MODULE_0
wire [7:0] aa = TEST_RTL_MODULE_0.DATA_I;
```
このコードで `TEST_RTL_MODULE_0.DATA_I` をパースすると `unbound object` エラーが出ていた。
一方、自動補完 (autocomplete) では `@scope` で登録されたTEST_RTL_MODULE_0のVirtualScopeNameSpaceが正しく参照できていた。

**原因**:
- `@scope` の `CommentScopeReferences` リストへの追加と `NamedElements` への VirtualScopeNameSpace の登録は、
  `VerilogFile.AcceptParsedDocumentAsync()` 内の `ApplyCommentScopeReferences()` で**パース完了後**に実行されていた
- 一方、`@scope` 付きコメントを前置する `wire` 宣言 (module_item) のパースはそれより**前**に実行されるため、
  同じ parse pass 内で `TEST_RTL_MODULE_0` がまだ `NamedElements` に登録されておらず `unbound object` になっていた
- 加えて、`CommentAnnotationItem.Parse` は `module_item` パースの**後**に呼ばれていたため、
  同じ `@scope` を前置する module_item のパース時にはまだ @scope が登録されていなかった

**修正内容**:
1. `Module.parseModule` のループで `CommentAnnotationItem.Parse` を `module_item` パースの**前**に呼び出すように変更
   - これにより、@scope を前置する module_item のパース時には @scope が既に登録済みになる
2. `CommentAnnotationItem.parseScopeAnnotation` で @scope を解析後、即座に VirtualScopeNameSpace を `NamedElements` に登録する処理を追加
   - 同一 parse pass で同じ @scope が再処理された場合の冪等性チェック付き
   - ターゲット BuildingBlock が未登録 (別ファイルで未パース) でも VirtualScopeNameSpace を仮登録し、`ReparseRequested = true` を立てる
3. `CommentScopeReference` に `VirtualScopeEntryName` プロパティを追加して既存エントリとの比較に使用
4. 既存の ApplyCommentScopeReferences 処理 (AcceptParsedDocumentAsync) も引き続き動作

**対応する構文**:
```verilog
// @scope buildingBlockName [instanceName]
// @scope buildingBlockName #(.paramName(value), ...) [instanceName]
```

例:
```verilog
// @scope TEST_RTL_MODULE TEST_RTL_MODULE_0
wire [7:0] aa = TEST_RTL_MODULE_0.DATA_I;   // 同じ parse pass でエラーなく解決
```

**修正ファイル**:
- `CodeEditor2VerilogPlugin/CodeEditor2VerilogPlugin/CodeEditor2VerilogPlugin/Verilog/BuildingBlocks/Module.cs`
- `CodeEditor2VerilogPlugin/CodeEditor2VerilogPlugin/CodeEditor2VerilogPlugin/Verilog/CommentScopeReference.cs`
- `CodeEditor2VerilogPlugin/CodeEditor2VerilogPlugin/CodeEditor2VerilogPlugin/Verilog/Items/CommentAnnotationItem.cs`

**ビルド結果**:
- `CodeEditor2VerilogPlugin.sln` ビルド成功 (828 警告, 0 エラー)
- `RtlEditor2.sln` 全体ビルドは DLL ロックにより失敗 (RtlEditor2.Desktop.exe が実行中)
  - ロックされたDLLは今回の変更とは無関係 (CodeEditor2AiPlugin, AjkAvaloniaLibs, AvaloniaEdit など)

**コミット**:
- サブモジュール CodeEditor2VerilogPlugin に commit 5fac078 として記録
